...
include_once($path.'/include/inc_basic-functions.php');
	...
	$_SESSION['page_id'] = mysql_real_escape_string($_GET['page_id']);
...
// erstelle neuen feed 
#exit;
$rss = new rss_feed("Shopste RSS Feed", "https://shopste.com/rss-feed/rss.xml", "Marktplatz für Händler und Einsteiger. Ermöglicht das Erstellen von Online Shops. Einfach nur registrieren und loslegen."); 
// Query 
$query = "SELECT * FROM modul_news_content ORDER BY AddDatum DESC LIMIT 0,20";
$resTextHTML =  mysql_query($query) or die(mysql_error());

while($strTEXTHTML = mysql_fetch_assoc($resTextHTML)) {
	$query = "SELECT * FROM menue JOIN domains ON menue.domain_id = domains.domain_id WHERE menue.id=".$strTEXTHTML['page_id']." AND hasPortalRSS='Y'";
	$resDomain = mysql_query($query) or die(mysql_error());
	$strDomain = mysql_fetch_assoc($resDomain);
	$strURL = 'http://'.$strDomain['name'].'/'.getPathUrl('de',$strTEXTHTML['page_id']); 
	#echo $strURL;
	#exit;
	$rss->add_item(htmlspecialchars($strTEXTHTML['AddTitel']), htmlspecialchars($strTEXTHTML['AddText']),$strURL);  
...
if(CORE_PIWIK_ACTIVE == 'YES') {
	...
	$t->doTrackPageView('Shopste Marktplatz und Webseiten Nachrichten');
