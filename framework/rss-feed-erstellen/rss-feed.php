...
// Query 
$query = "SELECT * FROM modul_texthtml ORDER BY lastchange DESC LIMIT 0,20";
...
while($strTEXTHTML = mysql_fetch_assoc($resTextHTML)) {
	...
	#echo $strURL;
	#exit;
	$rss->add_item(htmlspecialchars($strTEXTHTML['title_de']), htmlspecialchars($strTEXTHTML['content_de']),$strURL);  
	
	$date=$strTEXTHTML['lastchange']; //Date example
...
if(CORE_PIWIK_ACTIVE == 'YES') {
	...
	$t->doTrackPageView('Freie-Welt.eu letzte Inhalte');
