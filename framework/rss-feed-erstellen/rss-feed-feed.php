...
<link>".$this->textInput["link"]."</link> 
        ...
        $out .= "<atom:link href=\"https://".$_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"]."\" rel=\"self\" type=\"application/rss+xml\" />\r\n"; 
        $out .= $this->item_out(); 
        $out .= "</channel> 
</rss>"; 
        if($starttime !== FALSE) 
...
#exit; 
if(!empty($_GET['typ'])) {
	$strFields = explode("|",rawurldecode($_GET['typ']));
	if(is_numeric($strFields[1])) {
		$strIDS = getSubKategorie_rss(mysqli_real_escape_string(DBi::$conn,$strFields[1]),'',0);  
		$query = "SELECT * FROM modul_rss_content WHERE Bereich='".mysqli_real_escape_string(DBi::$conn,$strFields[1])."' ".$strIDS." ORDER BY created_at DESC LIMIT 0,50";
	#print_r($query);
	}
} else {
	$query = "SELECT * FROM modul_rss_content ORDER BY created_at DESC LIMIT 0,50";
}
if(!empty($strFields[0])) {
	$rss = new rss_feed("Freie Welt Nachrichtenportal ".$strFields[0], CORE_SERVER_DOMAIN.CORE_RSS_CONTENT_HTTP_PATH.$strFields[0].urlencode('|').$strFields[1], "Alle 15 Minuten neue Nachrichten aus $strFields[0]"); 
} else {
	$rss = new rss_feed("Freie Welt Nachrichtenportal ", CORE_SERVER_DOMAIN.CORE_RSS_CONTENT_HTTP_PATH, "Alle 15 Minuten neue Nachrichten."); 	
}
// Query 

$resTextHTML =  DBi::$conn->query($query) or die(mysqli_error(DBi::$conn));

while($strTEXTHTML = mysqli_fetch_assoc($resTextHTML)) {
	
	#echo "OK";
	$query = "SELECT * FROM menue JOIN domains ON menue.domain_id = domains.domain_id WHERE menue.id=".$strTEXTHTML['page_id'];
	$resDomain = DBi::$conn->query($query) or die(mysqli_error(DBi::$conn));
	$strDomain = mysqli_fetch_assoc($resDomain);
	$strURL = 'https://'.$strDomain['name'].'/'.getPathUrl('de',$strTEXTHTML['page_id']).'?utm_souce=Blog&amp;utm_medium=RSS&amp;utm_campaign=RSS'; 
	#echo $strURL;
	#exit;
	$rss->add_item(htmlspecialchars($strTEXTHTML['AddTitel']), htmlspecialchars($strTEXTHTML['AddText']),utf8_encode($strURL));  
	
	$date=$strTEXTHTML['created_at']; //Date example
	//echo $strTEXTHTML['created_at'];
	$strDateTime = explode(" ",$date);	
	$strDate = $strDateTime[0];
	$strDateSplit = explode("-",$strDate);
	
	$month = $strDateSplit[1]; 
	$year = $strDateSplit[0];
	$day = $strDateSplit[2];
	
	$strTimeSplit = explode(":",$strDateTime[1]);
	$hour = $strTimeSplit[0];
	$minute = $strTimeSplit[1];
	...
	$rss->item_set_guid(utf8_encode($strURL));
	#print_r($strTEXTHTML);
	#exit;
...
$t->doTrackPageView('RSS-FEED Request: '.$_GET['typ']);
