...
function getListMonthOrdersByDate($strDomain) {
	...
	$resOrderSelect = DBi::$conn->query($query) or die('ERR00001:'.$query.mysqli_error());
...
function setCustomerInvoiceEmail($strDomain) {
	...
	while($strDomain = mysqli_fetch_assoc($resDomains)) {
		...
		$resOrderSelect = DBi::$conn->query($query) or die(mysqli_error());
		$iActionPos = 0;
		while($strOrderSelect = mysqli_fetch_assoc($resOrderSelect)) {
		...
		if($iGebuehren['gebuehr'] != 0) {
			...
			$resRechnungExists = DBi::$conn->query($query) or die(mysqli_error());
			...
			$resShopInhaber = DBi::$conn->query($query) or die(mysqli_error());
...
case 'acp_show_hits':
		...
		case 'portal_umkreis_plz':
			...
			#echo $query;
			$resUmkreisErg = DBi::$conn->query($query) or die(mysqli_error());
			$bfound = false;
			$bCSV =true;
			$strTmpName ='';
			$strOutput ='';
		...
		case 'set_shopste_category_jtl':
			...
			$resInsert = DBi::$conn->query($query) or die('ER:000-4: '.mysqli_error());
			...
			#echo $query;
			$resPageIDRoot = DBi::$conn->query($query) or die('ER:000-3: '.mysqli_error());
			...
			$resInsert = DBi::$conn->query($query) or die('ER:000-2: '.mysqli_error());
			...
			$resInsert = DBi::$conn->query($query) or die('ER:000-1: '.mysqli_error());
			$iPageID = mysqli_insert_id(DBi::$conn);
			...
			$resInsert = DBi::$conn->query($query) or die('ER:0000: '.mysqli_error());
			...
			$resInsert = DBi::$conn->query($query) or die('ER:0001: '.mysqli_error());
			...
			$resInsert = DBi::$conn->query($query) or die('ER:0002: '.mysqli_error());
						#echo "N";
			...
			$resInsert = DBi::$conn->query($query) or die('ER:0003: '.$query.' '.mysqli_error());
			$iModulID = mysqli_insert_id(DBi::$conn);
			...
			$resInsert = DBi::$conn->query($query) or die('ER:0004'.mysqli_error());
		...
		case 'set_shopste_category':
			...
			# Kategorie Seiten ID auslesen
			$query ="SELECT * FROM shop_category WHERE shop_cat_id='".$_GET['shop_cat_id']."'";
			$resPageIDRoot = DBi::$conn->query($query) or die(mysqli_error());
			$strPageID = mysqli_fetch_assoc($resPageIDRoot);
		...
		case 'get_category_item':
			...
			while($data = mysqli_fetch_assoc($resShopItem)) {
				...
				$resBild = DBi::$conn->query($query) or die(mysqli_error());
		...
		case 'kontakt_send_mail':
			...
			#echo $query;
			$resKontaktModul = DBi::$conn->query($query) or die(mysqli_error());
...
case 'register_email_validate_blog':
		#echo "IN";
			...
			if(count($strBenutzer) != '') {
				...
				#echo $query;
				$resDomainOK = DBi::$conn->query($query)or die(myql_error());
	...
	// Startseite anlegen
		...
		#echo "IN";
			...
			$query = "SELECT * FROM benutzer WHERE email_crc='".$_GET['email_crc']."'";
			$resEmailCRC = DBi::$conn->query($query) or die(mysqli_error());
			$strBenutzer = mysqli_fetch_assoc($resEmailCRC);
			if(count($strBenutzer) != '') {
 ...
 #echo $html;
		...
		case 'portal_abrechnung_mahnung':
			...
			$resDomains = DBi::$conn->query($query) or die('ERR00001: '.mysqli_error());
 ...
 WHERE bezahlt_am is Null";
			$resUpdate = DBi::$conn->query($query) or die('ERR00002: '.mysqli_error());
			...
			while($strDomains = mysqli_fetch_assoc($resDomains)) {
				...
				#echo $query;
				$resMahnung = DBi::$conn->query($query) or die('ERR00003: '.mysqli_error());
			...
			switch($_GET['submodus']) {
				...
				case 'setStornoItem':
					...
					if($_GET['domain_crc'] == $strDomain['domain_crc'] && $_GET['domain_id'] == $strDomain['domain_id']) {
						...
						$resUpdate = DBi::$conn->query($query) or die('ERR0001:'.mysqli_error());;
...
<a id="action_pos_'.$iCount.'"></a>						
		...
		case 'core_gen_domain_id':
			...
			while($strDomains = mysqli_fetch_assoc($resDomains)) {
				...
				DBi::$conn->query($strQuery) or die('ERR00002:'.mysqli_error());;
			}
			break;			
		case 'cron_portal_abrechnung':			
		...
		case 'system_upload_file':
			...
			if(!empty($_POST['shopste_cat'])) {
				...
				$resInsert = DBi::$conn->query($query) or die('ER:0005'.mysqli_error()); 
				...
				if($shop_item_id_ary['shopste_marktplatz_menue_id'] == 0) {
					...
					#echo $query;
					$resCatTbl = DBi::$conn->query($query1) or die('ER:0001-1'.$query1.mysqli_error());
					...
					#echo $query;
					$resInsert = DBi::$conn->query($query2) or die('ER:0002'.$query1.' '.$query2.mysqli_error());
					...
					if($iPageID_shopste != '') {
						...
						DBi::$conn->query($query) or die('ER:0005'.mysqli_error());
					}
				}
			}
			break;
		...
		if($bLogedIn == true) {
				...
				if($strShopInfoCount['anzahl'] > 0) {
					...
					if($_POST['submodus'] == 'jtl2shopste') {
						...
						if($strShopInfo['shopste_marktplatz_cat'] == 0 && $_POST['shopste_cat'] != '') {
							...
							#echo $query;
							$resInsert = DBi::$conn->query($query) or die('ER:0002'.$query1.mysqli_error());
					...
					#mail("jbludau@bludau-media.de","Datei hochgeladen ".$_POST['shop_item_name'],$_POST['shop_item_beschreibung']);
					$resInsert = DBi::$conn->query($query) or die($query.'ER:0005'.mysqli_error()); 
					...
					if(!empty($_POST['shopste_cat']) && !empty($_POST['subshop_cat'])) {
						...
						#mail("jbludau@bludau-media.de","Datei hochgeladen ".$_POST['shop_item_name'],$_POST['shop_item_beschreibung']);
						$resInsert = DBi::$conn->query($query) or die('ER:0055'.mysqli_error()); 
						...
						#	echo "Shopste Adden";
							...
							#echo $query;
							$resCatTbl = DBi::$conn->query($query) or die('ER:0001-1'.$query.mysqli_error());
							$strMenueTbl = mysqli_fetch_assoc($resCatTbl);
							...
							$query = "INSERT INTO `menue_parent` (`menue_id`, `parent_id`) VALUES (".$iPageID_shopste.", ".$strMenueTbl['page_id'].");";
							#echo $query;
							$resInsert = DBi::$conn->query($query) or die('ER:0002-1'.$query.mysqli_error());
							...
							if($iPageID_shopste != '') {
								...
								$resModMenue = DBi::$conn->query($query) or die('ERR:008-0:'.$query.mysqli_error());
						...
						$query = "INSERT INTO `menue` (`name_de`, `titel_de`, `sortierung`, `status_de`, `layout`,domain_id,created_at,template_file) VALUES ('".$_POST['shop_item_name']."', '".$_POST['shop_item_name']."', '0', 'produktunsichtbar', 'col2-left-layout','".$_POST['domain_id']."','".date("Y-m-d H:i:s")."','index.tpl')";
						#echo $query;
						$resInsert = DBi::$conn->query($query) or die('ER:0006'.mysqli_error());
						...
						# Die Seiten ID der Kategorie ermitteln
							...
							#echo $query;
							$resCatTbl = DBi::$conn->query($query) or die('ER:0007'.mysqli_error());
							$strMenueTbl = mysqli_fetch_assoc($resCatTbl);
					# print_r($strMenueTbl);
						#$query = "INSERT INTO `menue_parent` (`menue_id`, `parent_id`) VALUES (".$iPageID.", ".$_POST['page_menue_id'].");";
						# Wird automatisch in dem "Pfad" des Menü gespeichert (übergeordnet ist die Kategorie id)
						...
						#echo $query;
						$resInsert = DBi::$conn->query($query) or die('ER:0008'.$query.mysqli_error());
						...
						$resInsert = DBi::$conn->query($query) or die('ER:0009: '.$query.' '.mysqli_error());
						$iModulID = mysqli_insert_id(DBi::$conn);
						...
						$resInsert = DBi::$conn->query($query) or die('ER:0010'.mysqli_error());
						...
						$resInsert = DBi::$conn->query($query) or die('ER:0007:'.mysqli_error());
						...
						$resModMenue = DBi::$conn->query($query) or die('ERR:008-1:'.$query.mysqli_error());
						$strCount= mysqli_fetch_assoc($resModMenue);
						...
						$_SESSION['acp_imported_ids'] .= $_SESSION['acp_imported_ids']."|".$shop_item_id."|";
							...
							#mail("jbludau@bludau-media.de","Datei hochgeladen ".$_POST['shop_item_name'],$_POST['shop_item_beschreibung']);
							$resInsert = DBi::$conn->query($query) or die('ER:1'.mysqli_error()); 
							$shop_item_id = mysqli_insert_id(DBi::$conn);
				...
				if($_POST['bLastItem'] == "true") {
					...
					for($i=0; $i < count($strIds); $i++) {
						...
						$resShopItem1 = DBi::$conn->query($query) or die(mysqli_error());
		...
		case 'system_category_add':
				...
				switch($_POST['subtyp']) {
					...
					case 'new':
						...
						$resCategory = DBi::$conn->query($query) or die(mysqli_error());
					...
					case 'rename':
						...
						$query = "SELECT * FROM shop_category WHERE shop_cat_id='".$_POST['shop_cat_id']."'";
						$resShopCat = DBi::$conn->query($query) or die(mysqli_error());
