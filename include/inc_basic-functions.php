...
function search_lastest($limit_results) {
	
	if(empty($limit_results)) {
		$limit_results = 6;
	}

	$query = "SELECT suchanfrage, suchanzahl FROM suche_anfragen WHERE freigeschaltet='Y' GROUP BY suchanfrage ORDER BY updated_at  LIMIT 0,".$limit_results;
...
/*
	if($bIn == true) {
		$strSearch = substr($strSearch,0,strlen($strSearch) - 2);
	}
*/
