...
function get_modulselector($params) {
	...
	function admin_cookie_check() {
		...
		# Pr�fen ob Cookie �berhaupt gesetzt
	    ...
	    #echo 				md5($i['password']).' -  '.$_COOKIE['admin_pwd'].' - '.$_COOKIE['admin_user'].' - '.$i['username'];
				...
				# Geladene Seite geh�rt nicht zur Domain
...
function getPageSettings($page_id) {
	...
	if (!empty($page_id)) {
		...
		$respage = DBi::$conn->query($query) or die(mysqli_error());
...
function getPageTitle($page_settings) { 
	...
	if($parentmenures3["titel_".$_SESSION['language']] != '') {
	...
	} else if ($parentmenures3["name_".$_SESSION['language']] != '') {
	...
	if($parentmenures["titel_".$_SESSION['language']] != '') {
	...
	} else if ($parentmenures["name_".$_SESSION['language']] != '') {
	...
	if($parentmenures2["titel_".$_SESSION['language']] != '') {
	...
	} else if ($parentmenures2["name_".$_SESSION['language']] != '') {
...
function getPathUrl($lang='de',$page) { 
	...
	$convertMenue = menue_NameConvert($res4['parent_id']);
	if($convertMenue['linktext']) { 
		$MenuePath[4] = utf8_encode(urlencode(strip_tags($convertMenue['linktext']))).'/'; 		
		unset($convertMenue); 
	}

	$res3 = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM menue_parent WHERE menue_id='$res4[parent_id]'"));
	$convertMenue = menue_NameConvert($res3['parent_id']);
	if($convertMenue['linktext']) { 
		$MenuePath[3] = utf8_encode(urlencode(strip_tags($convertMenue['linktext']))).'/'; 
		unset($convertMenue); 
	}

	$res2 = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM menue_parent WHERE menue_id='$res3[parent_id]'"));
	$convertMenue = menue_NameConvert($res2['parent_id']);
	if($convertMenue['linktext']) { 
		$MenuePath[2] = utf8_encode(urlencode(strip_tags($convertMenue['linktext']))).'/'; 
		unset($convertMenue); 
	}

	$res1 = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM menue_parent WHERE menue_id='$res2[parent_id]'"));
	$convertMenue = menue_NameConvert($res1['parent_id']);
	if($convertMenue['linktext']) { 
		$MenuePath[1] = utf8_encode(urlencode(strip_tags($convertMenue['linktext']))).'/'; 
		unset($convertMenue); 
	}

	$res0 = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM menue_parent WHERE menue_id='$res1[parent_id]'"));
	$convertMenue = menue_NameConvert($res0['parent_id']);	
	if($convertMenue['linktext']) { 
		$MenuePath[0] = utf8_encode(urlencode(strip_tags($convertMenue['linktext']))).'/'; 
		unset($convertMenue); 
	}
	$convertMenue = menue_NameConvert($page);
	$MenuePath[5] = utf8_encode(urlencode(strip_tags($convertMenue['linktext']))).'/';
...
function menue_generator($parent, $level,$html,$openli,$opencon,$typ='menue',$page_id='') { 
	...
	$yuiclass[2] = "bd";
	#yuimenubaritemlabel-hassubmenu 
	$yuiclass[3] = "flex-nav-item";
	#yuimenubaritemlabel-hassmenu-selected
	$yuiclass[4] = "flex-nav-item";
	#yuimenubaritemlabel-selected 
	$yuiclass[5] = "yuimenubaritem";
	$yuiclass[6] = "yuimenuitem";
	$yuiclass[7] = "yuimenu";
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		# URL mit absoluten Pfad 
		$url .= ' href="'.getPathUrl($_SESSION['language'],$row['id']).'?pk_campaign=nav-mainmenu&pk_kwd=nav-mainmenu&pk_source=nav-mainmenu" target="'.$row['target'].'" title="'.$titel.'">';
		...
		if($typ == 'menue') {
			$openli++;
			if($level == '0') { 
				$html .= '      <li id="menuItem_'.$row['menue_id'].'">'.$url;  	
			}
...
function getDomain($page_id) {
	...
	$res_domain = DBi::$conn->query($strQuery) or die(mysqli_error());	
...
function getDomainAry($domain_id) {
	...
	$resDomains = DBi::$conn->query($query) or die(mysqli_error());
...
function menue_NameConvert($menue_id) {
	...
	$resPageName = DBi::$conn->query($query) or die(mysqli_error());
...
if($convertMenue['name_de'] != '') {	
		if($bPlain == true) {
			...
			$MenuePath[4] = '<a property="item" typeof="WebPage" href="'.$rootpath.'/'.getPathUrl($_SESSION['domainLanguage'],$res4['parent_id']).'" title="'.    $convertMenue['name_de'].'"><span property="name"><i class="fas fa-angle-right"></i> '.$convertMenue['name_de'].'</span></a>'; unset($convertMenue); 	
		}
}
	$res3 = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM menue_parent WHERE menue_id='$res4[parent_id]'"));
	$convertMenue = convertMenueName($res3['parent_id']);
	if($convertMenue['name_de'] != '') {	
		if($bPlain == true && strlen($convertMenue['name_de']) > 0) {
			$MenuePath[3] = $convertMenue['name_de'];
			$strTrenner[3] = '-';
		} else {
			$MenuePath[3] = '<a property="item" typeof="WebPage" href="'.$rootpath.'/'.getPathUrl($_SESSION['domainLanguage'],$res3['parent_id']).'" title="'.$convertMenue['name_de'].'"><span property="name"><i class="fas fa-angle-right"></i> '.$convertMenue['name_de'].'</span></a>'; unset($convertMenue); 
		}
	}

	$res2 = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM menue_parent WHERE menue_id='$res3[parent_id]'"));
	$convertMenue = convertMenueName($res2['parent_id']);
	if($convertMenue['name_de'] != '') {
		if($bPlain == true && strlen($convertMenue['name_de']) > 0) {
			$MenuePath[2] = $convertMenue['name_de'];
			$strTrenner[2] = '-';
		} else {
			$MenuePath[2] = '<a property="item" typeof="WebPage" href="'.$rootpath.'/'.getPathUrl($_SESSION['domainLanguage'],$res2['parent_id']).'" title="'.$convertMenue['name_de'].'"><span property="name"><i class="fas fa-angle-right"></i> '.$convertMenue['name_de'].'</span></a>'; unset($convertMenue); 
		}
	}
	$res1 = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM menue_parent WHERE menue_id='$res2[parent_id]'"));
	$convertMenue = convertMenueName($res1['parent_id']);
	...
	if($convertMenue['name_de'] != '') {
		if($bPlain == true && strlen($convertMenue['name_de']) > 0) {
			...
			$MenuePath[1] = '<a property="item" typeof="WebPage" href="'.$rootpath.'/'.getPathUrl($_SESSION['domainLanguage'],$res1['parent_id']).'" title="'.$convertMenue['name_de'].'"><span property="name"><i class="fas fa-angle-right"></i> '.$convertMenue['name_de'].'</span></a>'; unset($convertMenue); 
		}
	}
	
	$res0 = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM menue_parent WHERE menue_id='$res1[parent_id]'"));
	$convertMenue = convertMenueName($res0['parent_id']);
	if($convertMenue['name_de'] != '') {
		if($bPlain == true && strlen($convertMenue['name_de']) > 0) {
			$MenuePath[0] = $convertMenue['name_de'];
			$strTrenner[0] = '-';
		} else {
			$MenuePath[0] = '<a property="item" typeof="WebPage" href="'.$rootpath.'/'.getPathUrl($_SESSION['domainLanguage'],$res0['parent_id']).'" title="'.$convertMenue['name_de'].'"><span property="name"><i class="fas fa-angle-right"></i> '.$convertMenue['name_de'].'</span></a>'; unset($convertMenue); 
		}
	}
	
	$convertMenue = convertMenueName($page);
	$MenuePath[5] = $convertMenue['name_de'];
		...
		$gesPath .= '<li property="itemListElement" typeof="ListItem"></li><i class="fas fa-angle-right"></i> Startseite <meta property="position" content="1"></li>';

		if(isset($MenuePath[0])) {
			$gesPath .= '<li property="itemListElement" typeof="ListItem"></li><i class="fas fa-angle-right"></i> '.$MenuePath[0].' <meta property="position" content="2"></li>';
		}
		if(isset($MenuePath[1])) {
			$gesPath .= '<li property="itemListElement" typeof="ListItem"></li><i class="fas fa-angle-right"></i> '.$MenuePath[1];
			if(isset($MenuePath[2])) {
				$gesPath .= ' > ';
			}
		...
		if(isset($MenuePath[2])) {
			$gesPath .= '<li property="itemListElement" typeof="ListItem"></li><i class="fas fa-angle-right"></i> '.$MenuePath[2];
			if(isset($MenuePath[3])) {
				$gesPath .= ' > ';
			}
		...
		if(isset($MenuePath[3])) {
			$gesPath .= '<li property="itemListElement" typeof="ListItem"></li><i class="fas fa-angle-right"></i> '.$MenuePath[3];
			if(isset($MenuePath[4])) {
				$gesPath .= ' > ';
			}
		...
		if(isset($MenuePath[4])) {
			$gesPath .= '<li property="itemListElement" typeof="ListItem"></li><i class="fas fa-angle-right"></i> '.$MenuePath[4];
			if(isset($MenuePath[5])) {
				$gesPath .= ' > ';
			}
			$gesPath .= ' <meta property="position" content="6"></li>';
		}
		if(isset($MenuePath[5])) {
			$gesPath .= '<li property="itemListElement" typeof="ListItem"></li><i class="fas fa-angle-right"></i> '.$MenuePath[5].'<meta property="position" content="7"></li>';
		}
...
function getIsMarketPlacePageID($marketplacepageid) {
	...
	$resIsMarketPlace =  DBi::$conn->query($query) or die(mysqli_error());
...
function getGebuehrenTotal($preis) {
	...
	$resGeb�hr =  DBi::$conn->query($query) or die(mysqli_error());
