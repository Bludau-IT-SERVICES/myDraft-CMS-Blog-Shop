...
function getPageTitle($page_settings) { 
	...
	if(!is_null($parentres['parent_id'])) {
		$parentmenures = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM menue WHERE id='$parentres[parent_id]'"));
	} else {
		$parentmenures = array();
	}
	
	if(!is_null($parentmenures['id'])) {
		$parentres2 = $parentres2 = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM menue_parent WHERE menue_id='$parentmenures[id]'"));
	} else { $parentres2 = array(); }
	
	if(!is_null($parentres2['parent_id'])) {	
		$parentmenures2 = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM menue WHERE id='$parentres2[parent_id]'"));
	} else {  $parentmenures2["titel_".$_SESSION['language']] = ''; }
	
	if(!is_null($parentmenures2['id'])) {	
		$parentres3 = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM menue_parent WHERE menue_id='$parentmenures2[id]'"));
	} else { $parentres3 = array(); } 
	
	if(!is_null($parentres3['parent_id'])) {	
		$parentmenures3 = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM menue WHERE id='$parentres3[parent_id]'"));
	} else { $parentmenures3["titel_".$_SESSION['language']] = ''; }
	...
	if(!is_null($parentmenures3["titel_".$_SESSION['language']])) {
	...
	} else if (!is_null($parentmenures3["name_".$_SESSION['language']])) {
	...
	if(!is_null($parentmenures["titel_".$_SESSION['language']])) {
	...
	} else if (!is_null($parentmenures["name_".$_SESSION['language']])) {
	...
	if(!is_null($parentmenures2['"titel_"'.$_SESSION['language']."'"])) {
	...
	} else if (!is_null($parentmenures2["name_".$_SESSION['language']])) {
...
function getPathUrl($lang='de',$page) {
	...
	if(!is_null($res3['parent_id'])) {		
		...
		if(!is_null($res2['parent_id'])) {
			$convertMenue = menue_NameConvert($res2['parent_id']);
			if($convertMenue['linktext']) { 
				$MenuePath[2] = $convertMenue['linktext'].'/'; 
				unset($convertMenue); 
			}
		}
	} else {
	...
	if(!is_null($res2['parent_id'])) {
		...
		if(!is_null($res1['parent_id'])) {
			$convertMenue = menue_NameConvert($res1['parent_id']);
			if($convertMenue['linktext']) { 
				$MenuePath[1] = $convertMenue['linktext'].'/'; 
				unset($convertMenue); 
			}
		}
	} else {
	...
	if(!is_null($res1['parent_id'])) {
		...
		if(!is_null($res0['parent_id'])) {
			$convertMenue = menue_NameConvert($res0['parent_id']);	
			if($convertMenue['linktext']) { 
				$MenuePath[0] = $convertMenue['linktext'].'/'; 
				unset($convertMenue); 
			}
		}
	} else {
	...
	$MenuePath[5] = $convertMenue['linktext'].'/';
...
function menue_generator($parent, $level,$html,$openli,$opencon,$typ='menue',$page_id='') { 
	...
	#	}
		...
		if($_GET['page_id'] == $row['id']) {
			#$page_settings = getPageSettings($row['id']); 
			if(@$_SESSION['login'] == 1 && $page_settings['fertig_de'] == '100') {
				$url .= '<img src="/images/page_done.png" border="0">';
			}
...
function menue_NameConvert($menue_id) {
	...
	if(!is_null($data["name_".$_SESSION['language']])) {
...
function convertMenueName($menue_id) {
	...
	$resErg = DBi::$conn->query($query) or die(mysqli_error());
