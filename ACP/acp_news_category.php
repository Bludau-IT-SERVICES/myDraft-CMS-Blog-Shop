<?php 
	...
	$strButtonName = 'News Kategorie Bereich anlegen';
	...
	if(isset($_POST['shop_cat_title'])) {
		...
		if($_POST['modus'] == 'new') {
			...
			$query = "INSERT INTO `modul_news_category` (`name_de`,created_at,sortierung,domain_id) VALUES ('".$_POST['shop_cat_title']."','".date("Y-m-d H:i:s")."','".$_POST['shop_cat_position']."','".$_SESSION['domain_id']."');";
			...
			$query = "INSERT INTO `modul_news_category_parent` (`news_cat_id`, `news_cat_parent`) VALUES (".$icat.", ".$_POST['shop_cat_id'].");";
			$resInsert = DBi::$conn->query($query) or die(mysqli_error());
			
			// Page Einstellugen Speichern
			$query = "INSERT INTO `menue` (`name_de`, `titel_de`, `sortierung`, `status_de`, `layout`,domain_id,content_type) VALUES ('".$_POST['page_url_name']."', '".$_POST['shop_page_titel']."', '".$_POST['shop_page_sort']."', 'sichtbar', '".$_POST['page_layout']."','".$_SESSION['domain_id']."','news_kategorie');";
			$resInsert = DBi::$conn->query($query) or die(mysqli_error());
			$iPageID = mysqli_insert_id(DBi::$conn);
			
			$query = "UPDATE modul_news_category SET page_id='".$iPageID."' WHERE news_cat_id='".$icat."'";
			...
			$query = "INSERT INTO `modul_news_categoryview` (`title_de`, `menue_id`, `last_usr`,news_cat) VALUES ('".$_POST['shop_page_titel']."', ".$iPageID.", 0,'".$icat."');";
			$resInsert = DBi::$conn->query($query) or die(mysqli_error());
			$iModulID = mysqli_insert_id(DBi::$conn);
		
			// Modul auf einer Seite bekannt machen
			$query = "INSERT INTO `module_in_menue` (`menue_id`, `modul_id`, `typ`,container,position) VALUES (".$iPageID.", ".$iModulID.", 'news_categoryview', 'col-main', '".$_POST['module_position']."');";
			$resInsert = DBi::$conn->query($query) or die(mysqli_error());
			...
			#@mail("info@shopste.com","News Kategorie angelegt: '".$_POST['shop_cat_title']."'",'LINK: '.$strLink);
			echo $strLink;
			exit;
		}	
		$query ="SELECT count(*) as anzahl FROM email_vorlage WHERE domain_id='".$_SESSION['domain_id']."' AND standard='N' AND typ='CORE_NEWS_CATEGORY_ADD'";
		#echo $query;
		...
		if($strEmailCount['anzahl'] > 0) {
			...
			$query ="SELECT * FROM email_vorlage WHERE domain_id='".$_SESSION['domain_id']."' AND standard='N' AND typ='CORE_NEWS_CATEGORY_ADD'";
			#echo $query;
			$resEmailVorlage = DBi::$conn->query($query) or die(mysqli_error());
			$strEmailVorlage = mysqli_fetch_assoc($resEmailVorlage);
		} else {
			# Lade Defaultvorlage
			$query ="SELECT * FROM email_vorlage WHERE domain_id='0' AND standard='Y' AND typ='CORE_NEWS_CATEGORY_ADD'";
			#echo $query;
			$resEmailVorlage = DBi::$conn->query($query) or die(mysqli_error());
			$strEmailVorlage = mysqli_fetch_assoc($resEmailVorlage);
		}
		...
		$betreff = str_replace('###NEWS_CAT_TITLE###',$_POST['page_url_name'],$betreff);
		if($_POST['modus'] == 'new') {
			$betreff = str_replace('###NEWS_MODUS###','hinzugefügt',$betreff);
		} else {
			$betreff = str_replace('###NEWS_MODUS###','aktuallisiert',$betreff);
		}
		$html = str_replace('###NEWS_CAT_TITLE###',$_POST['module_title'],$html);
		$html = str_replace('###NEWS_LINK###',CORE_SERVER_DOMAIN.$path,$html);
...
<div id="acp_main_new_domain_form">
	<h2>News Kategorie anlegen</h2>
	<form name="frmPageSetting" id="news_category" action="/ACP/acp_news_category.php" method="POST" onSubmit="return shop_save_form('news_category');">
		<div id="acp_new_page_form">
			<input type="hidden" id="acp_get_modus" name="modus" value="<?php echo $_GET['modus']; ?>"/>
			<div class="label" style="float:left;">News Bereich Bezeichnung*</div>
			...
			<div class="label" style="float:left;">News position</div>
