...
<?php
	...
	echo '<input type="hidden" id="acp_shop_itemlist_type" name="acp_shop_itemlist_type" value="lieferservice"/>';
	...
	<table width="100%" class="produkt_list_tbl">
		...
		while($data = mysqli_fetch_assoc($resQuery)) {
			...
			echo '<tr class="'.$cssClass.'">';
				...
				echo '<span id="acp_shop_list_edit" class="spanlink" onClick="getShopItemUpdate(\''.$data['shop_item_id'].'\')">Editieren</span> | <span id="acp_shop_list_edit" class="spanlink" onClick="set_shop_item_delete(\''.$data['shop_item_id'].'\')">l&ouml;schen</span> | <span id="acp_shop_list_activator" class="spanlink" onClick="set_shop_item_activator(\''.$data['shop_item_id'].'\',\''.$data['item_enabled'].'\')">'.$strEnabled.'</span>';
