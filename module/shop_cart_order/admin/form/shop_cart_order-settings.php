...
if(isset($_POST['optModul'])) {
	...
	if($_POST['modus'] == 'new') {
		$query = "INSERT INTO `modul_".$_POST['optModul']."` (`title_de`, `menue_id`, `last_usr`,option_lieferzeit) VALUES ('".$_POST['module_title']."', ".$_SESSION['page_id'].", 0,'".$_POST['option_lieferzeit']."');";
		...
		$query = "UPDATE `modul_".$_POST['optModul']."` SET title_de='".$_POST['module_title']."',option_lieferzeit='".$_POST['option_lieferzeit']."' WHERE id='".$_POST['module_id']."'";
		echo $query;
...
<?php 
	if($_GET['modus'] == 'new') { 
		echo '<h2>Neues Shop zur Kasse Modul <strong>hinzuf&uuml;gen</strong></h2>'; 
		require('../../../../include/inc_config-data.php');
	...
	if($_GET['modus'] == 'edit') {
		require('../../../../include/inc_config-data.php');
		echo '<h3>Shop zur Kasse Modul <strong>editieren</strong></h3>';
	  ...
	  <p>
			<form name="frmModulAdd" id="module_ajax_save_form" action="/module/<?= $_GET['module_name'].'/admin/form/'.$_GET['module_name'].'-settings.php'; ?>" method="POST" onSubmit="return module_save_form('module_ajax_save_form');">
				<div class="label" style="float:left;">
					<input type="hidden" id="acp_get_modul_name" name="optModul" value="shop_cart_order"/>
				...
				<div style="clear:both"></div>
				<div class="label" style="float:left;">Option Lieferzeit:</div>
				<div id="module_option_lieferzeit">
						<select name="option_lieferzeit" size="2">        
							<option value="N" <?php if($dataMenue['option_lieferzeit'] == 'N') echo "selected=true"?>>Lieferzeit ausblenden</option>
							<option value="Y" <?php if($dataMenue['option_lieferzeit'] == 'Y') echo "selected=true"?>>Lieferzeit einblenden</option>
						</select>			
				</div>		
				<div style="clear:both"></div>				
