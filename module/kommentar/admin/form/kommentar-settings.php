...
if(isset($_POST['optModul'])) {
	...
	if($_POST['modus'] == 'new') {
		$query = "INSERT INTO `modul_".$_POST['optModul']."` (`title_de`, `menue_id`, `last_usr`,content_de,created_at) VALUES ('".$_POST['module_title']."', ".$_COOKIE['last_page'].", 0, '".$_POST['module_texthtml_content']."','".date("Y-m-d H:i:s")."');";
		$resInsert = mysql_query($query) or die(mysql_error());
		...
		$query = "UPDATE `modul_".$_POST['optModul']."` SET title_de='".$_POST['module_title']."', content_de='".$_POST['module_texthtml_content']."' WHERE id='".$_POST['module_id']."'";
...
<?php 
	if($_GET['modus'] == 'new') { 
		echo '<h2>Letzte Nachrichten auf Freie-Welt.eu <strong>hinzuf&uuml;gen</strong></h2>'; 
		require('../../../../include/inc_config-data.php');
		$query = "SELECT max(position) as max_position FROM module_in_menue WHERE module_in_menue.menue_id =1";
		$resPosition = mysql_query($query) or die(mysql_error());
		$dataPosition = mysql_fetch_assoc($resPosition);
		$strButtonName = 'Modul hinzuf&uuml;gen';
		
	}
	if($_GET['modus'] == 'edit') {
		require('../../../../include/inc_config-data.php');
		echo '<h3>Letzte Nachrichten auf Freie-Welt.eu <strong>editieren</strong></h3>';
		
		$query = "SELECT * FROM modul_".$_GET['module_name']." WHERE id='".$_GET['id']."'";
		$resModul= mysql_query($query) or die(mysql_error());
		$dataMenue = mysql_fetch_assoc($resModul);
		
		$query = "SELECT * FROM module_in_menue WHERE modul_id='".$_GET['id']."' AND typ='".$_GET['module_name']."'";
		$resModulMenue = mysql_query($query) or die(mysql_error());
		$dataPageData = mysql_fetch_assoc($resModulMenue);
	  ...
	  <p>
			<form name="frmModulAdd" id="module_ajax_save_form" action="/module/<?= $_GET['module_name'].'/admin/form/'.$_GET['module_name'].'-settings.php'; ?>" method="POST" onSubmit="return module_save_form('module_ajax_save_form');">
				<div class="label" style="float:left;">
					<input type="hidden" id="acp_get_modul_name" name="optModul" value="news_latest"/>
