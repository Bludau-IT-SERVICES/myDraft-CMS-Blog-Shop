...
if(isset($_POST['optModul'])) {
	...
	if($_POST['modus'] == 'new') {
		$query = "INSERT INTO `modul_".$_POST['optModul']."` (`title_de`, `page_id`, `last_usr`,content_de,created_at) VALUES ('".$_POST['module_title']."', ".$_SESSION['page_id'].", 0, '".$_POST['module_texthtml_content']."','".date("Y-m-d H:i:s")."');";
...
<?php 
	if($_GET['modus'] == 'new') { 
		echo '<h2>Newsletter Modul <strong>hinzuf&uuml;gen</strong></h2>'; 
		require('../../../../include/inc_config-data.php');
	...
	if($_GET['modus'] == 'edit') {
		require('../../../../include/inc_config-data.php');
		echo '<h3>Newsletter Modul <strong>editieren</strong></h3>';
...
?>
	  ...
	  Stellt ein Formular für das hinzufügen von einem Newsletter Modul zur Verfügung. <br/>
	  Alle Anmeldungen werden in einer Newsletter Tabelle gespeichert. <br/>
			<form name="frmModulAdd" id="module_ajax_save_form" action="/module/<?= $_GET['module_name'].'/admin/form/'.$_GET['module_name'].'-settings.php'; ?>" method="POST" onSubmit="return module_save_form('module_ajax_save_form');">
				<div class="label" style="float:left;">
					<input type="hidden" id="acp_get_modul_name" name="optModul" value="newsletter_add"/>
