...
# >> TEXTHTML Modul 
####################################
function LoadModul_mekong_shop_speisekarte($config) {
	...
	$dataShopListe['typ'] = 'mekong_shop_speisekarte';
	#1171
	...
	$query ="SELECT * FROM shop_category LEFT JOIN shop_category_parent ON shop_category.shop_cat_id = shop_category_parent.id_shop_category_parent WHERE shop_category.domain_id = 61 AND NOT name_de='Alle Kategorien'";
	...
	while($strCats = mysqli_fetch_assoc($res)) {
		...
		$text .= '<div class=" grid_3  wow fadeInLeft">
			<div class="post grey-back maxheight grey_bottom_edge">
			<div class="post-image"><img src="images/page4_image1.jpg" alt="post image"></div>
			<div class="grey_top_edge">
			<h2 class="dark-2-title-2">Sandwiches</h2>';
		
		$query = "SELECT * FROM shop_item WHERE shop_cat_id='".$strCats['shop_cat_id']."'";
		$res2 = DBi::$conn->query($query) or die(mysqli_error());
		
		while($strItem = mysqli_fetch_assoc($res2)) {
			$text .= '<p>'.$strItem['name_de'].'<br/></p>';
		}
		$text .= '</div>';
		$text .= '</div>';
		$text .= '</div>';
