...
if(isset($_POST['optModul'])) {
	...
	if($_POST['modus'] == 'new') {
		$query = "INSERT INTO `modul_".$_POST['optModul']."` (`title_de`, `menue_id`, `last_usr`,content_de,created_at,news_cat) VALUES ('".$_POST['module_title']."', ".$_COOKIE['last_page'].", 0, '".$_POST['module_texthtml_content']."','".date("Y-m-d H:i:s")."','".$_POST['opt_news_cat_id']."');";
		...
		$query = "UPDATE `modul_".$_POST['optModul']."` SET title_de='".$_POST['module_title']."', content_de='".$_POST['module_texthtml_content']."' ,news_cat='".$_POST['opt_news_cat_id']."' WHERE id='".$_POST['module_id']."'";
...
<?php 
	if($_GET['modus'] == 'new') { 
		echo '<h2>Neues News Kategorie Modul <strong>hinzuf&uuml;gen</strong></h2>'; 
		#require('../../../../include/inc_config-data.php');
		...
		$strOptNewsKat = news_category(0, 0,'',$openli,$opencon,$typ='select',$page_id='');
	}
	if($_GET['modus'] == 'edit') {
		#require('../../../../include/inc_config-data.php');
		echo '<h3>News Kategorie Modul <strong>editieren</strong></h3>';
	  ...
	  <p>
			<form name="frmModulAdd" id="module_ajax_save_form" action="/module/<?= $_GET['module_name'].'/admin/form/'.$_GET['module_name'].'-settings.php'; ?>" method="POST" onSubmit="return module_save_form('module_ajax_save_form');">
				<div class="label" style="float:left;">
					<input type="hidden" id="acp_get_modul_name" name="optModul" value="news_categoryview"/>
				...
				<div id="module_menue_alphabetisch">
					<select name="opt_news_cat_id" size="1">        
