...
if(isset($_POST['optModul'])) {
	...
	if($_POST['modus'] == 'new') {
		...
		$query = "INSERT INTO `modul_".$_POST['optModul']."` (`title_de`, `menue_id`, `last_usr`,created_at,typ) VALUES ('".$_POST['module_title']."', ".$_SESSION['page_id'].", 0,'".date("Y-m-d H:i:s")."','".$_POST['module_typ']."');";
		...
		$query = "UPDATE `modul_".$_POST['optModul']."` SET title_de='".$_POST['module_title']."',typ='".$_POST['module_typ']."' WHERE id='".$_POST['module_id']."'";
	  ...
	  <p>
			<form name="frmModulAdd" id="module_ajax_save_form" action="/module/<?= $_GET['module_name'].'/admin/form/'.$_GET['module_name'].'-settings.php'; ?>" method="POST" onSubmit="return module_save_form('module_ajax_save_form');">
				<div class="label" style="float:left;">
					<input type="hidden" id="acp_get_modul_name" name="optModul" value="rss_content_popular"/>
				...
				<div id="module_position">
						<select name="module_typ" size="3">        
							<option value="DAY" <?php if($dataMenue['typ'] == 'DAY') echo "selected=true"?>>Tag</option>
							<option value="WEEK" <?php if($dataMenue['typ'] == 'WEEK') echo "selected=true"?>>Woche</option>
							<option value="MONTH" <?php if($dataMenue['typ'] == 'MONTH') echo "selected=true"?>>Monat</option>
						</select>			
				</div>		
				<div style="clear:both"></div>				
