...
if(isset($_POST['optModul'])) {
	...
	if($_POST['modus'] == 'new') {
		...
		if(empty($_POST['gui_show_news_content_preview'])) {
			$_POST['gui_show_news_content_preview'] = 'Y';
		}
		if(empty($_POST['gui_show_rss_content_preview'])) {
			$_POST['gui_show_rss_content_preview'] = 'Y';
		}
		if(empty($_POST['gui_show_shop_content_preview'])) {
			$_POST['gui_show_shop_content_preview'] = 'Y';
		}
		
		$query = "INSERT INTO `modul_".$_POST['optModul']."` (`title_de`, `menue_id`, `last_usr`,content_de,created_at,gui_show_news_content_preview,gui_show_rss_content_preview,gui_show_shop_content_preview) VALUES ('".$_POST['module_title']."', ".$_COOKIE['last_page'].", 0, '".$_POST['module_texthtml_content']."','".date("Y-m-d H:i:s")."','".$_POST['gui_show_news_content_preview']."','".$_POST['gui_show_rss_content_preview']."','".$_POST['gui_show_shop_content_preview']."');";
		...
		$query = "UPDATE `modul_".$_POST['optModul']."` SET title_de='".$_POST['module_title']."', content_de='".$_POST['module_texthtml_content']."',gui_show_rss_content_preview='".$_POST['gui_show_rss_content_preview']."',gui_show_news_content_preview='".$_POST['gui_show_news_content_preview']."',gui_show_shop_content_preview='".$_POST['gui_show_shop_content_preview']."' WHERE id='".$_POST['module_id']."'";
...
<?php 
	if($_GET['modus'] == 'new') { 
		echo '<h2>Neues Sitemap Modul <strong>hinzuf&uuml;gen</strong></h2>'; 
		require('../../../../include/inc_config-data.php');
	...
	if($_GET['modus'] == 'edit') {
		require('../../../../include/inc_config-data.php');
		echo '<h3>Sitemap Modul <strong>editieren</strong></h3>';
	  ...
	  <p>
			<form name="frmModulAdd" id="module_ajax_save_form" action="/module/<?= $_GET['module_name'].'/admin/form/'.$_GET['module_name'].'-settings.php'; ?>" method="POST" onSubmit="return module_save_form('module_ajax_save_form');">
				<div class="label" style="float:left;">
					<input type="hidden" id="acp_get_modul_name" name="optModul" value="sitemap"/>
				...
				<div class="label" style="float:left;">Zeige RSS-Kategorie News an</div>
				<div id="module_position">
						<select name="gui_show_rss_content_preview" size="2">        
							<option value="Y" <?php if($dataMenue['gui_show_rss_content_preview'] == 'Y') echo "selected=true"?>>Ja</option>
							<option value="N" <?php if($dataMenue['gui_show_rss_content_preview'] == 'N') echo "selected=true"?>>Nein</option>
						</select>			
				</div>		 
				...
				<div class="label" style="float:left;">Zeige Blog-Kategorie News an</div>
				<div id="module_position">
						<select name="gui_show_news_content_preview" size="2">        
							<option value="Y" <?php if($dataMenue['gui_show_news_content_preview'] == 'Y') echo "selected=true"?>>Ja</option>
							<option value="N" <?php if($dataMenue['gui_show_news_content_preview'] == 'N') echo "selected=true"?>>Nein</option>
						</select>			
				</div>		 

				
				<div style="clear:both"></div>			
				<div class="label" style="float:left;">Zeige Shop-Kategorie Artikel an</div>
				<div id="module_position">
						<select name="gui_show_shop_content_preview" size="2">        
							<option value="Y" <?php if($dataMenue['gui_show_shop_content_preview'] == 'Y') echo "selected=true"?>>Ja</option>
							<option value="N" <?php if($dataMenue['gui_show_shop_content_preview'] == 'N') echo "selected=true"?>>Nein</option>
						</select>			
				</div>		 
