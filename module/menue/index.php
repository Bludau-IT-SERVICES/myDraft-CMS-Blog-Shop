...
function LoadModul_menue($config) { 
	...
	if ($dataMenue["typ"] == 'menue') { 
		...
		#echo $query;
		$resMenuParent = DBi::$conn->query($query) or die(mysqli_error());
		...
		#echo $query.'<br/>';
		$menueqry = DBi::$conn->query($query) or die(mysqli_error()); 
	...
	if(!empty($menueqry)) {
		...
		if($bIn == false) {
				$query = "SELECT * FROM menue_parent WHERE menue_id='".$dataMenue["menue_id"]."'";
				$resMenue = DBi::$conn->query($query) or die(mysqli_error());
