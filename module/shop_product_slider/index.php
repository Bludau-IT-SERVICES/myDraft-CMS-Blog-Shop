...
function getSubKategorie($iParrentCat,$strIDs,$level,$dataTextHTML) {
	...
	while($strCatMenue = mysqli_fetch_assoc($resCat)) {	
		switch($dataTextHTML['cat_is_shop']) {
			case 'shop':
		...
		#echo $strIDs; 
		$strIDs = getSubKategorie($strCatMenue['shop_cat_id'],$strIDs,$level+1,$dataTextHTML);
...
# >> TEXTHTML Modul 
####################################
function LoadModul_shop_product_slider($config) {

		$dataTextHTML = mysqli_fetch_array(DBi::$conn->query("SELECT * FROM modul_shop_product_slider WHERE id=".$config['modul_id']));
		...
		$dataTextHTML['typ'] = 'modul_shop_product_slider';
        ...
        <div style="position: relative; left: 50%; width: 5000px; text-align: center; margin-left: -2500px;">
            ...
            <div id="'.$config['modul_id'].'_slider1_container" style="position: relative; margin: 0 auto;
                ...
                <div u="loading" style="position: absolute; top: 0px; left: 0px;">
                    ...
                    <div style="position: absolute; display: block; background: url(/framework/slider-master/img/loading.gif) no-repeat center center;
                        top: 0px; left: 0px; width: 100%; height: 100%;">
                    </div>
                </div>
                <!-- Slides Container -->
			...
			$ids = getSubKategorie($dataTextHTML['shop_category'],"",0,$dataTextHTML);
			$strColumZufall = '';
			...
			switch ($dataTextHTML['cat_is_shop']) {
				case 'shop':
					$query="SELECT * FROM shop_item WHERE shop_cat_id='".$dataTextHTML['shop_category']."' ".$ids." AND system_closed_shop='N' AND item_enabled='Y' ".$strColumZufall." LIMIT 0,".$dataTextHTML['produkt_anzahl'];
					break;
				case 'shopste':
			...
			while($ShopItem = mysqli_fetch_assoc($resShopItem)) {
				...
				$pathItem = 'http://'.$_SERVER['SERVER_NAME'].'/'.getPathUrl($_SESSION['language'],$ShopItem['menue_id']);
				$text .= '<div>';
				#echo $strBild['picture_url'];
                $text .= '
                <div>
                ...
                <style>
                    ...
                    */
                    .jssora07l, .jssora07r, .jssora07ldn, .jssora07rdn {
                        ...
                        cursor: pointer;
                        display: block;
                        background: url(/framework/slider-master/img/a07.png) no-repeat;
                        overflow: hidden;
                    }
                    .jssora07l {
